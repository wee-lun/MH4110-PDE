...
Again, it is empahsized that $\phi(x-ct)$ is travelling to the right, and $\phi(x+ct)$ is travelling to the left.
...
\begin{thm} [Solution formula for the diffusion equation] \label{thm: soln diff eqn}
    The problem 
    \[\begin{cases}
...
Due to the wide usage of the function $S(x,t)$, we shall formally define it: 
...
\begin{defn} [Gaussian kernel]
    ...
    is known as \textit{Gaussian kernel, fundamental solution, source function, Green's function,} or \textit{propagator} of the heat equation.
\end{defn}

It is direct to spot the following statement:

\medskip 

\begin{pro} [Evenness and Positivity of Gaussian kernel] \label{pro: evenness Gauss}
    Gaussian kernel $S(x,t)$ is an even function of $x$, and it is always positive.
\end{pro}

Essentially, the solution $u(x,t)$ is essentially a convolution of $S(x,t)$ with the initial value $\phi(x)$, which can be seen as a \textit{weighting function} that 'distribute' $\phi(y)$:
...
We now show that the parity of the solution $u(x,t)$ is the same as the parity of the initial value $\phi(x)$.

\medskip

\begin{lem} [Parity of solution for diffusion equation] \label{lem: parity soln diffusion}
    If the initial value $\phi(x)$ is even (odd), then the solution $u(x,t)$ is also even (odd) in $x$.
\end{lem}
\begin{proof}
    First suppose that $\phi(x)$ is even, i.e. $\phi(-x)=\phi(x)$ for all $x$. By previous theorem we have 
    \begin{align*}
        u(-x,t) &= \int_{-\infty}^{\infty} S(-x-y,t)\phi(y)\ dy\\
        &= -\int_{-\infty}^{\infty} S(-x+z,t)\phi(z)\ dz \quad \text{where } z:= -y\\
        &= \int_{-\infty}^{\infty} S(-x+z,t)\phi(z)\ dz \quad \text{since $\phi$ is even}\\
        &= \int_{-\infty}^{\infty} S(x-z,t)\phi(z)\ dz \quad \text{since $S$ is even by Proposition \ref{pro: evenness Gauss}}\\
    \end{align*}
    We see that $z$ is a dummy variable, so we can replace it by $y$ to get that $u(-x,t) = u(x,t)$, which shows that $u$ is even.

    Next, suppose that $\phi(x)$ is odd, i.e. $\phi(-x)=-\phi(x)$ for all $x$. Similarly, we start from 
    \begin{align*}
        u(-x,t) &= \int_{-\infty}^{\infty} S(-x-y,t)\phi(y)\ dy\\
        &= -\int_{-\infty}^{\infty} S(-x+z,t)\phi(z)\ dz \quad \text{where } z:= -y\\
        &= -\int_{-\infty}^{\infty} S(x-z,t)\phi(z)\ dz \quad \text{since $S$ is even by Proposition \ref{pro: evenness Gauss}}\\
    \end{align*}
    Again, we can replace $z$ by $y$ to get that $u(-x,t) = -u(x,t)$, which shows that $u$ is odd.
\end{proof}
...
\begin{re} \hfill
    \begin{itemize}
        ...
        \item Regardless of the smoothness of the initial condition $\phi(x)$, the solution $u(x,t)$ is sufficiently smooth for $t>0$ and any $x$, since all partial derivatives of $u$ exists.
    \end{itemize}
\end{re}
...
It is usually impossible to evaluate the solution $u(x,t)$ completely in terms of elementary functions. However, for some particular initial data $\phi(x)$, the solutions are expressible in terms of the \textit{error function}. 
...
A direct computataion tells that $\erf(0)=0$, and 
\[\lim_{x\to \infty} \erf(x) = \frac{2}{\sqrt{\pi}}  \int_0^\infty e^{-p^2}\ dp =\frac{2}{\sqrt{\pi}} \frac{\sqrt{\pi}}{2}= 1\]
We have in fact seen the error function previously. Recall that in the proof of Theorem \ref{thm: soln diff eqn}, we defined 
\[Q(x,t) = \frac{1}{2} + \frac{1}{\sqrt{\pi}}\int_0^{x/\sqrt{4kt}} e^{-s^2}\ dx\]
In fact we can write it as 
\[Q(x,t) = \frac{1}{2} + \frac{1}{2} \func{\erf}{\frac{x}{\sqrt{4kt}}}\]
...
\begin{ex}
    ...
    \[\begin{cases}
        ...
        \end{cases}
    \end{cases}\]
    We will evaluate its solution using the error function. By the solution formula we have 
    \begin{align*}
        u(x,t) 
        &= \frac{1}{\sqrt{4k\pi t}} \int_0^\infty \func{\exp}{-\frac{(x-y)^2}{4kt}}\ dy \\
        &= \frac{100}{\sqrt{\pi}} \int_{-x/\sqrt{4kt}}^\infty e^{-q^2}\ dq \quad \text{where we substitue } q:= \frac{y-x}{\sqrt{4kt}}\\
        &= \frac{100}{\sqrt{\pi}} \int_{0}^\infty e^{-q^2}\ dq + \frac{100}{\sqrt{\pi}} \int_{-x/\sqrt{4kt}}^0 e^{-q^2}\ dq \\
        &= \frac{100}{\sqrt{\pi}} \frac{\sqrt{\pi}}{2} + \frac{100}{\sqrt{\pi}} \int^{x/\sqrt{4kt}}_0 e^{-q^2}\ dq \\
        &= 50 + 50\func{\erf}{\frac{x}{\sqrt{4kt}}}
    \end{align*}
    where the second last equality is utilizing the fact that $e^{-q^2}$ is an even function.
\end{ex}

\section{Comparison of Waves and Diffusions}
We have learned that waves are essentially transporting the initial data in both directions at a finite speed. On the other hand, diffusions are spreading the initial datas in a smooth fashion and gradually disapper. Also, for wave equation, the energy is preserved as a constant, while in diffusion equation the energy decays to zero. They are also different from the perspective of well-posedness: wave equation is well-posed for all $t$, but diffusion equation is not for $t<0$. 

We end this chapter with an example demonstrating that Maximum Principle does not hold for the wave equation:

\medskip

\begin{ex}
    Consider the wave equation $u_{tt} = u_{xx}$ with initial condition 
    \[u(x,0) = 0 \qand u_t(x,0)=\psi(x) = \begin{cases}
        1,|x|<1\\
        0,|x|>1
    \end{cases}\]
    Suppose that the Maximum Principle holds for wave equation. Since in this case there is no lateral boundary, by the principle the maximum must be attained at $t=0$. However, when $t=0$, the initial condition gives $u(x,0)=0$, and based on d'Alembert formula the solution is positive for $t>0$. This shows that maximum principle does not hold for wave equation. 
\end{ex}
